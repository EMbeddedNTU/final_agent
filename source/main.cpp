...
const static char WIFI_SSID[] = "MakerSpace_2.4G";
...
void polling() {
    ...
    SharedPtr<GSH::HttpService::HttpResponse> response =
        http_service.http_get("http://192.168.10.64:3000/agent/1", NULL);
    GSH_TRACE("%s %d", response->body, num);
...
int main() 
{
    ...
    SharedPtr<GSH::HttpService::HttpResponse> response = http_service.http_post(
        "http://192.168.10.64:3000/agent/register", NULL, msg);
