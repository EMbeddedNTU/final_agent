...
HttpService::http_request(char *http_headers, SharedPtr<ParsedUrl> purl) {
  /* Parse url */
  if (!purl) {
...
*/
HttpService::HttpResponse *HttpService::http_get(const char *url,
                                                 char *custom_headers) {
  /* Parse url */
...
*/
HttpService::HttpResponse *
HttpService::http_post(const char *url, char *custom_headers, char *post_data) {
  /* Parse url */
  SharedPtr<ParsedUrl> purl = ParsedUrl::create(url);
  if (purl == nullptr) {
    GSH_ERROR("Unable to parse url");
